...
[`zarf`](https://zarf.dev/) is a CLI tool written in Go for airgapping Kubernetes deployments.  Facing a potential use case where a user may be very unfamiliar with deploying K8s, given minimal training, and a requirement that the job must be completed; a GUI for this powerful tool needed to be created.
