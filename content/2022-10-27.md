...
where `name` is the name of the image (statically `library/registry` in this case), and `reference` is the tag of the image (in this case we don't care what `reference` is, we will only be serving one image for every reference).
...
But wait, we don't have a v2 manifest!  We have a Crane manifest from our `seed-image.tar` [Crane tarball format](https://github.com/google/go-containerregistry/blob/main/pkg/v1/tarball/README.md).
