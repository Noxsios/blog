...
__EmptyDir__ does not have the ability to `chmod` itself upon instantiation, but __ConfigMaps__ do!
...
So, only tests matching `src/test/e2e/[00-19]_*_test.go` (_this is not real regex_) should be run.
...
- `examples/component-scripts` was broken because the scripts used `touch`, which does not exist on Windows
- `src/cmd/package.go` needed some regex adjusted to allow `\` and `:` as valid characters in Zarf's cache path
...
>
> Windows support is still in alpha, but it is now possible to run Zarf on Windows!
