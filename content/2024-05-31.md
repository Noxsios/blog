...
To accomplish this, I clobbered together some pretty ugly glue code to allow for this new behavior to exist within component composition, but that introduced some new and nasty bugs while also exacerbating many others.
...
The entire design of `ImportChain` revolves around the principle that all node in the chain have a one:one relationship with each other. Each node is a component that satisfies the `flavor` and `arch` of the previous node.
...
**BUT** that is only if we decide to keep "skeleton" packages around. They are a pretty niche feature, and I think there is potential that UDS bundles + more experienced package creators have superseded their usefulness. Musings for another day.
