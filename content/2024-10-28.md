...
We wouldn't want to have to re-upload a 40GB image (thanks for nothing Windows) every time we change a line of code right?
...
but the flexibility of this string is what allows for OCI to be able to store arbitrary content like:
...
`Size` (in bytes) serves as the first component in [content verification](https://github.com/opencontainers/distribution-spec/blob/main/spec.md#content-verification) (making sure the content being passed around hasn't been corrupted or tampered with).
