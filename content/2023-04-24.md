...
It is worth mentioning that in essence, Zarf component actions are really just scripts w/ some special sauce cooked up by Jeff.
...
This means that if you have access to a OCI registry, you can move files around with Zarf (don't forget to take advantage of upload/download concurrency w/ the `--oci-concurrency <n>` flag).
